...
function SaveLaterCard({ product, cartDispatch }) {
	...
	function decreaseHandler() {
		if (product.quantity !== 1)
			cartDispatch({ type: 'DECREASE_SAVE_ITEM_QUANTITY', payload: { id: product.id } });
		else {
			cartDispatch({ type: 'REMOVE_FROM_LATER', payload: { product } });
		}
	}
		...
		<div className='w-48 h-80 mt-0 ml-0 cursor-pointer shadow-sm transition-shadow duration-300 ease-in-out rounded-sm hover:shadow-lg md:w-64 md:h-96 md:m-4 md:ml-0 md:mt-0'>
			...
			<div className='flex items-center mx-2 mt-2 max-w-max border-2 border-blue-500 rounded-sm'>
				...
				<button
					onClick={() =>
						cartDispatch({
							type: 'INCREASE_SAVE_ITEM_QUANTITY',
			...
			<button
				...
				className='bg-blue-600 text-white px-3 py-2 text-xs mt-2 mx-2 rounded-md font-medium transition-colors hover:bg-blue-700'
			>
				REMOVE
			</button>
			<button
